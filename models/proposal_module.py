...
class ProposalModule(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, num_proposal, sampling,
        ...
        self.conv1 = torch.nn.Conv1d(proposal_size, proposal_size, 1)
        self.conv2 = torch.nn.Conv1d(proposal_size, proposal_size, 1)
        self.bn1 = torch.nn.BatchNorm1d(proposal_size)
        self.bn2 = torch.nn.BatchNorm1d(proposal_size)
        # JUST FOR

        if self.position_type == 'seed_attention':
            self.seed_feature_trans = torch.nn.Sequential(
                torch.nn.Conv1d(seed_feat_dim, proposal_size, 1),
                torch.nn.BatchNorm1d(proposal_size),
                torch.nn.PReLU(proposal_size)
            )

        self.detr = DETR3D(config_transformer, input_channels=proposal_size, class_output_shape=2+num_class, bbox_output_shape=3+num_heading_bin*2+num_size_cluster*4+int(quality_channel))
