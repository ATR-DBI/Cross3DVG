...
def get_eval(data_dict, config, reference, use_lang_classifier=False, use_oracle=False, use_cat_rand=False,
    ...
    if use_best:
        # cluster_labels: 8, 1, 256
        pred_ref = torch.argmax(data_dict["cluster_labels"][:,0], 1)
        data_dict['cluster_ref'] = data_dict["cluster_labels"][:,0]
    if use_cat_rand:
        ...
        for i in range(cluster_preds.shape[0]):
            ...
            candidate_masks = torch.gather(sem_cls_label == data_dict["object_cat_list"][i][0], 
                                           0, data_dict["object_assignment"][i])
    ...
    ref_sem_label_list = data_dict["ref_sem_label_list"]
    gt_ref_sem_labels = []
    ...
    for i in range(batch_size):
        ...
        for j in range(len_nun_max):
            if j < lang_num[i]:
                ...
                gt_bbox = construct_bbox_corners(gt_obb[0:3], gt_obb[3:6]) # batch, 3
                pred_bboxes.append(pred_bbox)
                gt_bboxes.append(gt_bbox)
                gt_ref_sem_labels.append(ref_sem_label_list[i][j].item()) #.detach().cpu().numpy())
                # construct the multiple mask
                multiple.append(data_dict["unique_multiple_list"][i][j].item())
                # construct the others mask
                flag = 1 if data_dict["object_cat_list"][i][j] == config.type2class['others'] else 0
                others.append(flag)
    # lang
    ...
    data_dict["ref_sem_labels"] = gt_ref_sem_labels
