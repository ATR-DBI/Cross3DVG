        ...
        for scene in data:
            for scans in scene["scans"]:
                if "transform" in scans:
                    rescan2ref[scans["reference"]] = np.array(scans["transform"]).reshape(4,4).T # match scannet
    return rescan2ref
...
def write_bbox(bbox, mode, output_file):
    ...
    palette = {
        0: [0, 255, 0], # gt [green]
        1: [0, 0, 255], # pred (target) [blue]
        2: [255, 0, 0], # pred (comparison) [red]
    }
...
def align_mesh_scannet(scene_id, MESH, META):
    vertices, faces = read_mesh(MESH.format(scene_id, scene_id))
    for line in open(META.format(scene_id, scene_id)).readlines():
    ...
    pts[:, :3] = vertices[:, :3]
    #pts = np.dot(axis_align_matrix, pts)
...
def align_mesh_rio(scene_id, MESH, rot_matrix, ref_rot_matrix):
    vertices, faces = read_mesh(MESH.format(scene_id))
    r_points = vertices[:, :3].transpose()
    ...
    vertices[:, :3] = r_points.transpose()[:, :3]
    mesh = export_mesh(vertices, faces)
    return mesh
...
def dump_results(args, dataloader, scanrefer, data, config, CONF, MESH, META, pred_mode=1):
    dump_dir = os.path.join(CONF.PATH.OUTPUT, args.folder, "vis", args.dataset)
    os.makedirs(dump_dir, exist_ok=True)
    
    if args.dataset == 'riorefer':
        meta_file = os.path.join(META, '3RScan.json')
        rescan2ref = read_transform_matrix(meta_file)    
        reference_dic = get_reference_dic(meta_file)
        with open(os.path.join(META, 'reference_axis_align_matrix.pkl'), 'rb') as f:
    ...
    # reference
    gt_ref_labels = data["ref_box_label_list"][:,0].detach().cpu().numpy() 
    ...
    for i in range(batch_size):
        ...
        if not os.path.exists(scene_dump_dir):
            ...
            if args.dataset == 'scanrefer':
                mesh = align_mesh_scannet(scene_id, MESH, META)
                ...
                mesh = align_mesh_rio(scene_id, MESH, rot_matrix, ref_rot_matrix)
            else:
                raise NotImplementedError
        ...
        pred_file_prefix = 'pred_{}_{}_{}_{:.5f}_{:.5f}'.format(object_id, object_name, ann_id, pred_ref_scores_softmax[i, pred_ref_idx], iou)
        write_bbox(pred_obb, pred_mode, os.path.join(scene_dump_dir, pred_file_prefix + '.ply'))
        # dump image on a box
        if 'pred_box_frame_ids' in data:
            pred_box_img = get_box_image(args, CONF, dataloader, data, i, chunk_ind=0)        
            pred_box_img.save(os.path.join(scene_dump_dir, pred_file_prefix + '.jpg'))
        
        
def get_box_image(args, CONF, dataloader, data_dict, batch_ind=0, chunk_ind=0):
    record = dataloader.dataset.scanrefer_new[data_dict["scan_idx"][batch_ind]][chunk_ind] # scan_idx, chunk_idx (lang_num_max)    
    ...
    for i, frame_id in enumerate(pred_box_frame_ids):
        if args.dataset == 'scanrefer':
            frame_file = os.path.join(CONF.PATH.FRAME_SQUARE, record['scene_id'], 'color', str(int(frame_id))+'.jpg')
            ...
            frame_file = os.path.join(CONF.PATH.SCANS, record['scene_id'], 'sequence', 'frame-'+str(int(frame_id)).zfill(6)+'.color.jpg')
...
def visualize(args, DC, pred_mode=1):
    if args.dataset == "scanrefer":
        from lib.scanrefer_config import CONF
        SCANS_ROOT = CONF.PATH.SCANS # TODO point this to your scannet data
        MESH = os.path.join(SCANS_ROOT, "{}/{}_vh_clean_2.ply") # scene_id, scene_id 
        META = os.path.join(SCANS_ROOT, "{}/{}.txt") # scene_id, scene_id 
        ...
        SCANS_ROOT = CONF.PATH.SCANS # TODO point this to your scannet data
        MESH = os.path.join(SCANS_ROOT, "{}/mesh.refined.v2.color.ply") # scene_id  
        META = CONF.PATH.META
    ...
    for data in tqdm(dataloader):
        ...
        # visualize
        dump_results(args, dataloader, scanrefer, data, DC, CONF, MESH, META, pred_mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred_mode", type=int, help="", default=1) # 1 blue, 2 red
    ...
    visualize(args, DC, args.pred_mode)
